...
if [ -n "$CASSANDRA_SERVER" ]; then
    ...
    sed -i "/\[DEFAULTS\]/a cassandra_server_list = $CASSANDRA_SERVER_LIST" /etc/contrail/contrail-device-manager.conf
fi

if [ -n "$DISCOVERY_SERVER" ]; then
    sed -i "s/disc_server_ip = 127.0.0.1/disc_server_ip = $DISCOVERY_SERVER/g" /etc/contrail/contrail-device-manager.conf
fi

if [ -n "$RABBIT_SERVER" ]; then
    sed -i "/\[DEFAULTS\]/a rabbit_server = $RABBIT_SERVER" /etc/contrail/contrail-device-manager.conf
fi

if [ -n "$CONFIG_API_SERVER" ]; then
    sed -i "/\[DEFAULTS\]/a api_server_ip = $CONFIG_API_SERVER" /etc/contrail/contrail-device-manager.conf
fi
...
if [ -n "$ZOOKEEPER_SERVER" ]; then
    ...
    sed -i "/\[DEFAULTS\]/a zk_server_ip = $ZOOKEEPER_SERVER_LIST" /etc/contrail/contrail-device-manager.conf
fi
